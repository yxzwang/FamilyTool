...
def build_graph(input: list):
    ...
    for i in range(len(input)):
        if input[i] == "IF":
            found_if = True
            if_id = i
        elif found_if and input[i] == "THEN":
            then_id = i
            couple_list.append([if_id, then_id, -1])
            found_if = False
        elif input[i] == "ELSE":
            else_id = i
            couple_list[-1][2] = else_id
    ...
    then_id_list = [couple[1] for couple in couple_list]
    while "THEN" in input and input.index("THEN") not in then_id_list:
        i = input.index("THEN")
    ...
    while 1:
        if i < len(input):
            if input[i] =="THEN" and i not in then_id_list:
                G.add_edge(input[i-1], input[i+1], description="True")
                input.pop(i)
                input.pop(i)
            else:
    ...
    [if_id, then_id, else_id] = couple_list[0]
    for [if_id, then_id, else_id] in couple_list:
        G.add_edge(input[if_id+1], input[then_id+1], description="True")
        if else_id != -1:
            G.add_edge(input[if_id+1], input[else_id+1], description="False")
...
if __name__ == "__main__":
    ...
    expression_1 = "IF A THEN B ELSE C IF D THEN E ELSE F"
